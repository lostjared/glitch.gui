...
extern "C" void proc(cv::Mat  &frame) {
    ...
    static double r[3] = {static_cast<double>(rand()%10),static_cast<double>(rand()%10),static_cast<double>(rand()%10)};
    static double inc[3] = {0.01 * r[0], 0.01 * r[1], 0.01 * r[2]};
    for(int q = 0; q < 3; ++q) {
        if(dir[q] == 1) {
            alpha[q] += inc[q];
            if(alpha[q] >= 3.0) {
